  ...
  <v-dialog class="tw-text-center" width="364" v-model="value">
    <v-card>
      <div class="tw-p-8 tw-flex tw-flex-col tw-gap-6">
        <div>
          <div class="tw-text-left tw-mb-2 tw-font-bold tw-text-sm">ステータス名</div>
          <BaseText placeholder="ステータス" v-model="data.name"></BaseText>
        </div>
        <div>
          <div class="tw-text-left tw-mb-2 tw-font-bold tw-text-sm">ステータスカラー</div>
...
async function update() {
  await fetchData(`api/bc/master/statuses/${props.data.id}`, 'put', props.data);
}
</script>
